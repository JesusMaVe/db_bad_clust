...
SAMPLE_CURRENCIES = ["USD", "EUR", "MXN", "dólares", "euros", "pesos", "$", "$USD", None, ""]
...
def generate_poorly_designed_tables() -> list[AntiPatternTable]:
    ...
    for i, letter in enumerate(letters):
        cols_abc[letter] = "VARCHAR2(4000)"
    for i in range(26, 40):
        cols_abc[f"aa{chr(ord('a') + i - 26)}" if i < 52 else f"ab{chr(ord('a') + i - 52)}"] = (
            "VARCHAR2(4000)"
        )

    tables.append(
        AntiPatternTable(
            name="TABLA_BASE_DATOS",
            columns=cols_abc,
            no_primary_key=True,
            inconsistent_naming=True,
            giant_table=True,
    ...
    cols_backup = {}
    for i in range(1, 51):
        cols_backup[f"COL_{i:03d}"] = "VARCHAR2(4000)"
    tables.append(
        AntiPatternTable(
            name="BACKUP_DATOS",
            columns=cols_backup,
    ...
    tables.append(
        AntiPatternTable(
            ...
            columns={
                "ID": "NUMBER",
                "DESCRIPCION": "LONG",
                "DATOS_BINARIOS": "LONG RAW",
                "NOMBRE_LARGO": "NVARCHAR2(2000)",
                "CODIGO_HEX": "RAW(2000)",
                "FLAG_ACTIVO": "CHAR(2000)",
                "FECHA_REGISTRO": "DATE",
                "COMENTARIOS": "LONG",
            },
            no_primary_key=True,
    ...
    tables.append(
        AntiPatternTable(
            ...
            columns={
                "ID_VENTA": "NUMBER",
                "PRODUCTO_FK": "VARCHAR2(20)",
                "CLIENTE_NOMBRE": "VARCHAR2(100)",
                "CANTIDAD": "NUMBER",
                "PRECIO_TOTAL": "VARCHAR2(30)",
                "FECHA_VENTA": "VARCHAR2(30)",
                "VENDEDOR": "VARCHAR2(100)",
            },
            no_primary_key=True,
            wrong_data_types=True,
            fk_constraints={
                "PRODUCTO_FK": "PRODUCTOS(SKU) CASCADE",
            },
        )
    )
    ...
    tables.append(
        AntiPatternTable(
            ...
            columns={
                ...
                "EMAIL": "VARCHAR2(100)",
                "INTENTOS_FALLIDOS": "VARCHAR2(10) DEFAULT 'NADA'",
                "ACTIVO": "VARCHAR2(5) DEFAULT 0",
                "FECHA_EXPIRACION": "VARCHAR2(30) DEFAULT '9999-12-31'",
                "ROL": "VARCHAR2(20) DEFAULT 12345",
                "ULTIMO_ACCESO": "VARCHAR2(30)",
            },
            no_primary_key=True,
            wrong_data_types=True,
        )
...
def _generate_generic_dirty_data(table: AntiPatternTable, num_rows: int) -> list[dict[str, Any]]:
    col_list = list(table.columns.items())
    rows = []
    for i in range(1, num_rows + 1):
        row = {}
        for col_name, col_type in col_list:
            t = col_type.upper()
            if any(x in t for x in ("NUMBER", "FLOAT", "INTEGER", "INT", "NUMERIC")):
                row[col_name] = random.choice([i, i + 1000, None, -i, 9999, 0])
            ...
            elif any(x in t for x in ("BLOB", "CLOB", "RAW", "LONG")):
                row[col_name] = None
            else:
                row[col_name] = _generate_bad_text(f"x{i}")
        rows.append(row)
    return rows


def generate_bad_data_for_table(table: AntiPatternTable) -> AntiPatternData:
    num_rows = random.randint(150, 400)
    ...
    gen = specific.get(table.name)
    if gen:
        rows = gen(num_rows)
    else:
        rows = _generate_generic_dirty_data(table, num_rows)
...
def _generate_bad_empleados_data(num_rows: int) -> list[dict[str, Any]]:
    ...
    for i in range(1, num_rows + 1):
        ...
        if contradictory and activo in ("No", "0", "false"):
            fecha_alta = (datetime.now() + timedelta(days=random.randint(1, 30))).strftime(
                "%Y-%m-%d"
            )
...
def _generate_bad_todo_en_uno_data(num_rows: int) -> list[dict[str, Any]]:
    ...
    for i in range(1, num_rows + 1):
        ...
        if tipo == "CLIENTE":
            ...
            cant_o_precio = random.choice(
                [f"${random.randint(10, 999)}", f"{random.randint(10, 999)}€", "Consultar", None]
            )
            ...
            nombre_o_desc = nombre
            cant_o_precio = random.choice(
                [f"${random.randint(20000, 80000)}", f"{random.randint(20000, 80000)} euros", None]
            )
        ...
        rows.append(
            {
                "ID": i if random.random() > 0.3 else None,
                "TIPO_REGISTRO": tipo,
                "NOMBRE_O_DESCRIPCION": _generate_bad_text(nombre_o_desc)
                if isinstance(nombre_o_desc, str)
                else nombre_o_desc,
                "CANT_O_PRECIO": str(cant_o_precio)
                if not isinstance(cant_o_precio, str)
                else cant_o_precio,
                "FECHA_O_DIRECCION": str(fecha_o_dir)
                if not isinstance(fecha_o_dir, str)
                else fecha_o_dir,
                "ESTADO_O_ACTIVO": str(estado) if estado is not None else None,
                "OBSERVACIONES": random.choice(
                    [None, "", "<b>Urgente</b>", "  Pendiente  ", "Verificado®™"]
                ),
                ...
                "REPETIDA_1": nombre_o_desc
                if isinstance(nombre_o_desc, str)
                else str(nombre_o_desc),
            }
        )
    return rows
...
def _generate_bad_configuracion_data(num_rows: int) -> list[dict[str, Any]]:
    configs = [
        ...
        {
            "clave": "logoBase64",
            "valor": "iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAADUlEQVR42mNk+M9QDwADhgGAWjR9awAAAABJRU5ErkJggg==",
            "tipo": "BASE64",
        },
